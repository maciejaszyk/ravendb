...
public class IncludeScoresAndDistancesCorax(ITestOutputHelper output) : RavenTestBase(output)
{
    ...
    private IDocumentStore PrepareData()
    {
        ...
        {
            record.Settings[RavenConfiguration.GetKey(x => x.Indexing.CoraxExposeDocumentDistance)] = true.ToString();
            record.Settings[RavenConfiguration.GetKey(x => x.Indexing.CoraxExposeDocumentScore)] = true.ToString();
        };
