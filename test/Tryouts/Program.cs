...
public static class Program
{
    ...
    {
        using FileStream csvFile = File.Create(@"C:\perf\MapReduceOptimized.csv");
        ...
        var queries = session.Query<QueryGenerated>()
            .Where(i => i.AverageReqTime >= 100 && i.AverageReqTime <= 250).OrderBy(i => i.AverageReqTime, OrderingType.Long)
            .ToList()
            .Take(50)
            .Select(i => new KeyValuePair<string, string>(i.Id, i.Query));//.Select(i => i.Replace("Questions/Search", "QuestionSearchCustom")).ToList();
        ...
        {
            var count = innerSession.Advanced.RawQuery<QuestionTags>(q.Value).NoCaching().NoTracking().Statistics(out var stats).Count();
    ...
    public static void GenerateMapReduceQueries()
    {
        ...
        Parallel.For(0, 10_000, (i) =>
        {
            ...
            if (leftSide == entries.Count) goto Repeat;
            ...
            if (innerQuery.Count == 0) goto RightSideRepeat;
    ...
    public static void InsertQueriesIntoRaven()
    {
        ...
        using var session = DocumentStoreHolder.Store.OpenSession(new SessionOptions {NoCaching = true});
        using (StreamReader sr = new StreamReader(@"C:\Users\macie\Downloads\reqs_files\questions_search.reqs"))
        {
            while (sr.Peek() >= 0)
            {
    ...
    public static void SaveQueriesIntoFile()
    {
        ...
        using FileStream csvFile = File.Create(@"C:\perf\queries.csv");
        using var writer = new StreamWriter(csvFile);
        using var csvWriter = new CsvWriter(writer, CultureInfo.CurrentCulture);
