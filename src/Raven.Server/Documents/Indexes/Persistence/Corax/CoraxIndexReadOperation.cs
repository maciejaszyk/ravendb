                ...
                where TQueryFilter : struct, ISupportsQueryFilter
        {
            ...
            while (runQuery)
            {
                ...
                if (builderParameters.HasBoost || hasOrderByDistance)
                {
                    sortingData = new()
                    {
                        ScoresBuffer = _index.Configuration.CoraxExposeDocumentScore && builderParameters is {HasBoost: true}
                            ? ScorePool.Rent(bufferSize)
                            : null,
                        DistancesBuffer = _index.Configuration.CoraxExposeDocumentDistance && hasOrderByDistance
                ...
                while (true)
                {
                    ...
                    Include:
                        ...
                        var retrieverInput = new RetrieverInput(IndexSearcher, _fieldMappings, indexEntryReader, key, _index.IndexFieldsPersistence, documentScore, documentDistance);
