...
namespace Raven.Server.Documents.Handlers
{
    public sealed class QueriesHandler : DatabaseRequestHandler
    {
        ...
        public Task Post()
        {
            return new DatabaseQueriesHandlerProcessorForGet(this, HttpMethod.Post).ExecuteAsTaskAsync();
        }

        [RavenAction("/databases/*/queries", "GET", AuthorizationStatus.ValidUser, EndpointType.Read, DisableOnCpuCreditsExhaustion = true)]
        public Task Get()
        {
            return new DatabaseQueriesHandlerProcessorForGet(this, HttpMethod.Get).ExecuteAsTaskAsync();
        }
